    ...
    <el-form ref="form" label-width="100px" :model="tm" :rules="rules">
      ...
      <el-form-item label="正确答案：" prop="answer" required>
        <el-radio-group v-model="tm.answer">
          <el-radio v-for="(item, index) in tm.optionsTmpArr" :key="index" :label="optionNumberToLetter[item.key]" border>{{ optionNumberToLetter[item.key] }}</el-radio>
        </el-radio-group>
      </el-form-item>
  ...
  methods: {
    initData() {
      if (this.tiMuData.title) {
        let data = []
        const dataPoint = []
        if (Array.isArray(this.tiMuData.ext.options) && this.tiMuData.ext.options.length > 0) {
          this.tiMuData.ext.options.forEach((v, i) => {
        ...
        this.tm = {
          ...
          optionsTmpArr: data,
          options: this.tiMuData.ext.options,
          knowledge_point_id: dataPoint
        }
      } else {
        this.tm = {
          type: this.$store.getters.timuType.DAN_XUAN,
          difficulty_id: 1,
          title: '',
          answer: [],
          parse: '',
    ...
    submitForm(formName) {
      if (this.tm.title === '') {
        this.$message.warning('请填写题目')
      } else if (Object.keys(this.pointIdArr).length === 0) {
