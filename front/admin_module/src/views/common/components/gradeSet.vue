  ...
  <div class="gradeSet">
    ...
    <div class="box-shadow bg-white pb-20">
      ...
      >
        ...
        <el-table-column label="层次" width="" align="center" show-overflow-tooltip>
          <template slot-scope="{row}">
            <template v-if="row.edit">
              <el-input v-model="row.grade" size="mini" />
            </template>
...
<script>
// import { fetchListGradeSet, handleAddGradeSet, handleDelGradeSet } from '@/api/objKu'
...
export default {
  ...
  methods: {
    getList() {
      ...
      /* fetchListGradeSet().then(response => {
      ...
      this.list = [
        { num: '01', grade: '高职', edit: false },
        { num: '02', grade: '中职', edit: false },
        { num: '03', grade: '本科', edit: false }
      ]
      this.list.push({
        difficulty: '',
        edit: true
    ...
    saveEdit(row) {
      if (row.difficulty === '') {
        ...
        handleAddGradeSet(data).then(response => {
    ...
    handleDelOne(id) {
      if (id !== undefined) {
        this.$confirm('确定要删除当前适用层次吗?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          /* handleDelGradeSet({ id: id }).then(response => {
    ...
    handleDel() {
      if (this.multipleSelection.length > 0) {
        this.$confirm('确定要删除吗?', '提示', {
          ...
          handleDelGradeSet({ id: tmpString }).then((response) => {
            this.$message.success('删除成功')
            this.getList()
          })*/
        }).catch(() => {
